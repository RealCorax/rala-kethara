---
title: "Rala'skela'thael — The 35 Portal Symbols"
cssclass: archive-content archive-data archive-wide
branch: 4
section: "04.01.06b"
access: visitor
type: technical-spec
tags: [technology, portal, language, Furling]
---

> [!record-header]
> Meva'shala · Compiled from dialling records, Era 14–148 · Visitor

# <span class="furling-pin">Rala'skela'thael</span> | The Star-Cycle-Marks

> *Varathal named the first twelve symbols in Era 14, working from the stellar survey data that had just come back from the outer reaches of the Kigal Galaxy. The choice to use sounds rooted in the language rather than arbitrary notation was not unanimous among the engineers. Varathal's position was simple: we are building something that will outlast everyone who builds it. It should speak.*
> — Archive note, compiler unknown, Era 19

>> The thirty-five symbols of the Furling portal network. Each corresponds to a stellar formation visible from the core of the Kigal Galaxy. Each sound is drawn from the roots of the Furling language. An address is six of these sounds spoken in sequence. The seventh sound identifies the portal you are dialling from.

<style>
.symbol-grid {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(160px, 1fr));
  gap: 16px;
  margin: 2em 0;
}
.symbol-card {
  background: var(--arc-bg-raised, #111820);
  border: 1px solid var(--arc-stone, rgba(180,155,110,0.12));
  border-radius: 10px;
  padding: 16px 12px 12px 12px;
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 8px;
  transition: border-color 0.2s ease;
}
.symbol-card:hover {
  border-color: var(--arc-teal-dim, rgba(74,184,196,0.30));
}
.symbol-card img {
  width: 72px;
  height: 72px;
  object-fit: contain;
  /* Dark mode: invert black strokes to near-white */
  filter: invert(1) opacity(0.75);
}
.theme-light .symbol-card img {
  /* Light mode: keep black strokes, just lower opacity slightly */
  filter: invert(0) opacity(0.85);
}
.symbol-number {
  font-size: 0.62em;
  letter-spacing: 0.14em;
  text-transform: uppercase;
  color: var(--arc-amber, #c8913a);
  opacity: 0.7;
}
.symbol-sound {
  font-size: 1.15em;
  font-weight: 600;
  color: var(--arc-text, #ddd4c0);
  letter-spacing: 0.04em;
}
.symbol-pronunciation {
  font-size: 0.78em;
  color: var(--arc-text-muted, #9e9080);
  font-style: italic;
}
.symbol-meaning {
  font-size: 0.75em;
  color: var(--arc-text-muted, #9e9080);
  text-align: center;
  line-height: 1.4;
}
.symbol-ring {
  font-size: 0.62em;
  letter-spacing: 0.1em;
  text-transform: uppercase;
  color: var(--arc-teal, #4ab8c4);
  opacity: 0.6;
  margin-top: 2px;
}
.ring-header {
  font-size: 0.71em;
  font-weight: 700;
  letter-spacing: 0.15em;
  text-transform: uppercase;
  color: var(--arc-amber, #c8913a);
  opacity: 0.8;
  margin: 2em 0 0.75em 0;
  border-bottom: 1px solid var(--arc-stone, rgba(180,155,110,0.12));
  padding-bottom: 0.5em;
}
</style>

## How to Read This Reference

The three rings of a portal dial each contain a subset of the symbols. The inner ring activates when the key is inserted. The middle and outer rings appear holographically when the key's side buttons are pressed in sequence.

Each symbol's **sound** is spoken when dialling from memory. The **root echo** shows the Furling language root each sound was drawn from. The **meaning carried** is what a Furling hears beneath the navigation code — not required for dialling, but present in every address spoken aloud.

---

<p class="ring-header">Inner Ring — 12 Symbols</p>
<p style="font-size:0.85em; color:var(--arc-text-muted); margin-bottom:1.5em; font-style:italic;">Most commonly dialled destinations. Local systems, major hubs. Activates when key is inserted.</p>

<div class="symbol-grid">

<div class="symbol-card">
<div class="symbol-number">01</div>
<img src="/rala-kethara/star-symbols/1.svg" alt="Symbol vel">
<div class="symbol-sound">vel</div>
<div class="symbol-pronunciation">vehl</div>
<div class="symbol-meaning">life, living</div>
<div class="symbol-ring">Inner</div>
</div>

<div class="symbol-card">
<div class="symbol-number">02</div>
<img src="/rala-kethara/star-symbols/2.svg" alt="Symbol om">
<div class="symbol-sound">om</div>
<div class="symbol-pronunciation">ohm</div>
<div class="symbol-meaning">peace, undisturbed harmony</div>
<div class="symbol-ring">Inner</div>
</div>

<div class="symbol-card">
<div class="symbol-number">03</div>
<img src="/rala-kethara/star-symbols/3.svg" alt="Symbol sha">
<div class="symbol-sound">sha</div>
<div class="symbol-pronunciation">shah</div>
<div class="symbol-meaning">wisdom, knowing</div>
<div class="symbol-ring">Inner</div>
</div>

<div class="symbol-card">
<div class="symbol-number">04</div>
<img src="/rala-kethara/star-symbols/4.svg" alt="Symbol mir">
<div class="symbol-sound">mir</div>
<div class="symbol-pronunciation">meer</div>
<div class="symbol-meaning">earth, home ground</div>
<div class="symbol-ring">Inner</div>
</div>

<div class="symbol-card">
<div class="symbol-number">05</div>
<img src="/rala-kethara/star-symbols/5.svg" alt="Symbol eth">
<div class="symbol-sound">eth</div>
<div class="symbol-pronunciation">ehth</div>
<div class="symbol-meaning">life at its eternal edge</div>
<div class="symbol-ring">Inner</div>
</div>

<div class="symbol-card">
<div class="symbol-number">06</div>
<img src="/rala-kethara/star-symbols/6.svg" alt="Symbol kel">
<div class="symbol-sound">kel</div>
<div class="symbol-pronunciation">kehl</div>
<div class="symbol-meaning">source, origin point</div>
<div class="symbol-ring">Inner</div>
</div>

<div class="symbol-card">
<div class="symbol-number">07</div>
<img src="/rala-kethara/star-symbols/7.svg" alt="Symbol ven">
<div class="symbol-sound">ven</div>
<div class="symbol-pronunciation">vehn</div>
<div class="symbol-meaning">living flow</div>
<div class="symbol-ring">Inner</div>
</div>

<div class="symbol-card">
<div class="symbol-number">08</div>
<img src="/rala-kethara/star-symbols/8.svg" alt="Symbol tha">
<div class="symbol-sound">tha</div>
<div class="symbol-pronunciation">thah</div>
<div class="symbol-meaning">time, pattern</div>
<div class="symbol-ring">Inner</div>
</div>

<div class="symbol-card">
<div class="symbol-number">09</div>
<img src="/rala-kethara/star-symbols/9.svg" alt="Symbol ren">
<div class="symbol-sound">ren</div>
<div class="symbol-pronunciation">rehn</div>
<div class="symbol-meaning">help, movement toward</div>
<div class="symbol-ring">Inner</div>
</div>

<div class="symbol-card">
<div class="symbol-number">10</div>
<img src="/rala-kethara/star-symbols/10.svg" alt="Symbol vai">
<div class="symbol-sound">vai</div>
<div class="symbol-pronunciation">vah-ee</div>
<div class="symbol-meaning">toward, becoming</div>
<div class="symbol-ring">Inner · Nibiru origin</div>
</div>

<div class="symbol-card">
<div class="symbol-number">11</div>
<img src="/rala-kethara/star-symbols/11.svg" alt="Symbol shu">
<div class="symbol-sound">shu</div>
<div class="symbol-pronunciation">shoo</div>
<div class="symbol-meaning">deep wisdom</div>
<div class="symbol-ring">Inner</div>
</div>

<div class="symbol-card">
<div class="symbol-number">12</div>
<img src="/rala-kethara/star-symbols/12.svg" alt="Symbol nai">
<div class="symbol-sound">nai</div>
<div class="symbol-pronunciation">nah-ee</div>
<div class="symbol-meaning">water, living landscape</div>
<div class="symbol-ring">Inner</div>
</div>

</div>

---

<p class="ring-header">Middle Ring — 11 Symbols</p>
<p style="font-size:0.85em; color:var(--arc-text-muted); margin-bottom:1.5em; font-style:italic;">Extended network. Outer systems, allied worlds. Appears on first side button press.</p>

<div class="symbol-grid">

<div class="symbol-card">
<div class="symbol-number">13</div>
<img src="/rala-kethara/star-symbols/13.svg" alt="Symbol ath">
<div class="symbol-sound">ath</div>
<div class="symbol-pronunciation">ahth</div>
<div class="symbol-meaning">ancient, originary</div>
<div class="symbol-ring">Middle</div>
</div>

<div class="symbol-card">
<div class="symbol-number">14</div>
<img src="/rala-kethara/star-symbols/14.svg" alt="Symbol mor">
<div class="symbol-sound">mor</div>
<div class="symbol-pronunciation">mohr</div>
<div class="symbol-meaning">voiced peace, speaking harmony</div>
<div class="symbol-ring">Middle</div>
</div>

<div class="symbol-card">
<div class="symbol-number">15</div>
<img src="/rala-kethara/star-symbols/15.svg" alt="Symbol sel">
<div class="symbol-sound">sel</div>
<div class="symbol-pronunciation">sehl</div>
<div class="symbol-meaning">keeper, tender</div>
<div class="symbol-ring">Middle</div>
</div>

<div class="symbol-card">
<div class="symbol-number">16</div>
<img src="/rala-kethara/star-symbols/16.svg" alt="Symbol veth">
<div class="symbol-sound">veth</div>
<div class="symbol-pronunciation">vehth</div>
<div class="symbol-meaning">life at its boundary</div>
<div class="symbol-ring">Middle</div>
</div>

<div class="symbol-card">
<div class="symbol-number">17</div>
<img src="/rala-kethara/star-symbols/17.svg" alt="Symbol kor">
<div class="symbol-sound">kor</div>
<div class="symbol-pronunciation">kohr</div>
<div class="symbol-meaning">completion, full path</div>
<div class="symbol-ring">Middle</div>
</div>

<div class="symbol-card">
<div class="symbol-number">18</div>
<img src="/rala-kethara/star-symbols/18.svg" alt="Symbol mev">
<div class="symbol-sound">mev</div>
<div class="symbol-pronunciation">mehv</div>
<div class="symbol-meaning">gift, sustaining</div>
<div class="symbol-ring">Middle</div>
</div>

<div class="symbol-card">
<div class="symbol-number">19</div>
<img src="/rala-kethara/star-symbols/19.svg" alt="Symbol rha">
<div class="symbol-sound">rha</div>
<div class="symbol-pronunciation">rhah</div>
<div class="symbol-meaning">deep time, ceremonial weight</div>
<div class="symbol-ring">Middle</div>
</div>

<div class="symbol-card">
<div class="symbol-number">20</div>
<img src="/rala-kethara/star-symbols/20.svg" alt="Symbol thal">
<div class="symbol-sound">thal</div>
<div class="symbol-pronunciation">thahl</div>
<div class="symbol-meaning">guardian, concealment</div>
<div class="symbol-ring">Middle</div>
</div>

<div class="symbol-card">
<div class="symbol-number">21</div>
<img src="/rala-kethara/star-symbols/21.svg" alt="Symbol elu">
<div class="symbol-sound">elu</div>
<div class="symbol-pronunciation">eh-loo</div>
<div class="symbol-meaning">sky-breath, open air</div>
<div class="symbol-ring">Middle</div>
</div>

<div class="symbol-card">
<div class="symbol-number">22</div>
<img src="/rala-kethara/star-symbols/22.svg" alt="Symbol nal">
<div class="symbol-sound">nal</div>
<div class="symbol-pronunciation">nahl</div>
<div class="symbol-meaning">voice of water, flowing speech</div>
<div class="symbol-ring">Middle</div>
</div>

<div class="symbol-card">
<div class="symbol-number">23</div>
<img src="/rala-kethara/star-symbols/23.svg" alt="Symbol sko">
<div class="symbol-sound">sko</div>
<div class="symbol-pronunciation">skoh</div>
<div class="symbol-meaning">belonging, rooted place</div>
<div class="symbol-ring">Middle</div>
</div>

</div>

---

<p class="ring-header">Outer Ring — 12 Symbols</p>
<p style="font-size:0.85em; color:var(--arc-text-muted); margin-bottom:1.5em; font-style:italic;">Deep network. Remote systems, waypoint installations. Some may now be inactive. Appears on second side button press.</p>

<div class="symbol-grid">

<div class="symbol-card">
<div class="symbol-number">24</div>
<img src="/rala-kethara/star-symbols/24.svg" alt="Symbol lha">
<div class="symbol-sound">lha</div>
<div class="symbol-pronunciation">lhah</div>
<div class="symbol-meaning">vast, awe-scale</div>
<div class="symbol-ring">Outer</div>
</div>

<div class="symbol-card">
<div class="symbol-number">25</div>
<img src="/rala-kethara/star-symbols/25.svg" alt="Symbol lhor">
<div class="symbol-sound">lhor</div>
<div class="symbol-pronunciation">lhor</div>
<div class="symbol-meaning">vast peace, silence between stars</div>
<div class="symbol-ring">Outer</div>
</div>

<div class="symbol-card">
<div class="symbol-number">26</div>
<img src="/rala-kethara/star-symbols/26.svg" alt="Symbol keth">
<div class="symbol-sound">keth</div>
<div class="symbol-pronunciation">kehth</div>
<div class="symbol-meaning">eternal, without boundary</div>
<div class="symbol-ring">Outer · see note</div>
</div>

<div class="symbol-card">
<div class="symbol-number">27</div>
<img src="/rala-kethara/star-symbols/27.svg" alt="Symbol thel">
<div class="symbol-sound">thel</div>
<div class="symbol-pronunciation">thehl</div>
<div class="symbol-meaning">sky-guardian, held air</div>
<div class="symbol-ring">Outer</div>
</div>

<div class="symbol-card">
<div class="symbol-number">28</div>
<img src="/rala-kethara/star-symbols/28.svg" alt="Symbol oru">
<div class="symbol-sound">oru</div>
<div class="symbol-pronunciation">oh-roo</div>
<div class="symbol-meaning">deep peace, interior</div>
<div class="symbol-ring">Outer</div>
</div>

<div class="symbol-card">
<div class="symbol-number">29</div>
<img src="/rala-kethara/star-symbols/29.svg" alt="Symbol ves">
<div class="symbol-sound">ves</div>
<div class="symbol-pronunciation">vehs</div>
<div class="symbol-meaning">life belonging</div>
<div class="symbol-ring">Outer</div>
</div>

<div class="symbol-card">
<div class="symbol-number">30</div>
<img src="/rala-kethara/star-symbols/30.svg" alt="Symbol yel">
<div class="symbol-sound">yel</div>
<div class="symbol-pronunciation">yehl</div>
<div class="symbol-meaning">breath-light</div>
<div class="symbol-ring">Outer</div>
</div>

<div class="symbol-card">
<div class="symbol-number">31</div>
<img src="/rala-kethara/star-symbols/31.svg" alt="Symbol shel">
<div class="symbol-sound">shel</div>
<div class="symbol-pronunciation">shehl</div>
<div class="symbol-meaning">wisdom carried on air</div>
<div class="symbol-ring">Outer</div>
</div>

<div class="symbol-card">
<div class="symbol-number">32</div>
<img src="/rala-kethara/star-symbols/32.svg" alt="Symbol reth">
<div class="symbol-sound">reth</div>
<div class="symbol-pronunciation">rehth</div>
<div class="symbol-meaning">help-life, the act of reaching</div>
<div class="symbol-ring">Outer</div>
</div>

<div class="symbol-card">
<div class="symbol-number">33</div>
<img src="/rala-kethara/star-symbols/33.svg" alt="Symbol alo">
<div class="symbol-sound">alo</div>
<div class="symbol-pronunciation">ah-loh</div>
<div class="symbol-meaning">ancient peace</div>
<div class="symbol-ring">Outer</div>
</div>

<div class="symbol-card">
<div class="symbol-number">34</div>
<img src="/rala-kethara/star-symbols/34.svg" alt="Symbol vhei">
<div class="symbol-sound">vhei</div>
<div class="symbol-pronunciation">vhee</div>
<div class="symbol-meaning">quiet becoming</div>
<div class="symbol-ring">Outer</div>
</div>

<div class="symbol-card">
<div class="symbol-number">35</div>
<img src="/rala-kethara/star-symbols/35.svg" alt="Symbol miru">
<div class="symbol-sound">miru</div>
<div class="symbol-pronunciation">mee-roo</div>
<div class="symbol-meaning">ground below ground, deep world</div>
<div class="symbol-ring">Outer</div>
</div>

</div>

> [!arc-note] Symbol 26 — Keth — Navigator's Note
> The stellar formation that symbol **keth** was mapped to has undergone significant change since Era 14. The symbol remains in use but navigators are advised to confirm destination coordinates independently before dialling any address containing keth as one of its six destination symbols.

> [!arc-note] Symbol 10 — Vai — Nibiru's Origin Symbol
> Symbol **vai** is Nibiru's origin symbol, chosen by Bulutha before the city was closed to general travel. Every address dialled from Nibiru ends with *vai*. The future particle. The word that means something is turning toward what it will become. Every time anyone dials out from this city, the last sound they speak is hope.

> [!connections] Connected Records
> [[Thevan-skela-shala|Thevan'skela'shala]] — portal network operations and history
> [[Thevan-skela-vel|Thevan'skela'vel]] — practical user guide for first-time portal travel
> [[04-Mevashala/index|Meva'shala]] — return to the branch index

*Rala'kethara Archive · Meva'shala · Access: Visitor*
